  ...
  <div class="shadow-lg">
    <form @submit.prevent="hanldeRegister">
      ...
      <div class="flex flex-col">
        ...
        />
        <span :class="inputPassword ? 'done' : 'error'"
          >Vui lòng nhập password</span
        >
      </div>
      <div class="flex flex-col">
        <label>Tuổi:</label>
        <input v-model="age" type="text" placeholder="vd:18" />
        <span :class="inputAge ? 'done' : 'error'"
          >Bé tí tuổi ranh học không lo học</span
        >
      </div>
      <div class="flex flex-col">
        <p class="text-[1.0rem] font-[500]">
          Bạn có phải là fan chị Min không?
        </p>
        ...
        <span :class="inputOption ? 'done' : 'error'"
          >Không hâm mộ thì lướt</span
        >
      </div>
      <div class="items-center text-center">
        <button
          class="px-8 py-4 mb-5 mx-[auto] rounded-lg text-base font-medium border-1px border-[#999] bg-[#FFC5DA]"
        >
          Đăng kí
        </button>
      </div>
  ...
  methods: {
    ...
    hanldeRegister() {
      ...
      if (
        ...
        this.$store.commit('hanldeRegister', {
          name: this.name,
